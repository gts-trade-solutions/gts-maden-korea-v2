...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (userIds.length) {
      ...
      // Email lives on the auth user row (auth_users, keyed by the same id as
      // profiles). One batched read instead of per-id lookups.
    ...
    return json(jsonSafe({ ok: true, range, filter, sessions: filtered.slice(0, limit) }));
  } catch (e: any) {
