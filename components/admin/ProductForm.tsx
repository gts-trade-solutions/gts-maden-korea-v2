...
async function bulkDownloadTemplate() {
  ...
  async function bulkUploadAssets() {
    ...
    await mapLimit(tasks, limit, async ({ key, file }) => {
      try {
        await uploadMedia("product-media", key, file, {
          upsert: bulkOverwriteImages,
        });
      } catch (e: any) {
        throw new Error(`${key}: ${e?.message || String(e)}`);
      }
      done += 1;
  ...
  async function bulkUpsertAll(): Promise<{ ok: boolean; issues: string[] }> {
    ...
    try {
      ...
      for (let i = 0; i < bulkProducts.length; i++) {
        ...
        try {
          const prodRow = await adminWrite({
            table: "products",
            op: "upsert",
            data: payload,
            onConflict: "slug",
          ...
          if (!prodRow?.id) {
            issues.push(`Upsert failed '${label}': no id returned`);
            continue;
          }
          product_id = prodRow.id as string;
        } catch (e: any) {
          issues.push(`Upsert failed '${label}': ${e?.message || String(e)}`);
          continue;
        }
        ...
        if (bulkReplaceImages) {
          try {
            await adminWrite({
              ...
              op: "delete",
              match: { product_id },
              mirrorScope: product_id,
            });
          } catch (e: any) {
            issues.push(
              `Image cleanup failed '${label}': ${e?.message || String(e)}`
            );
          }
        }
        ...
        if (imgs.length) {
          ...
          try {
            ...
            issues.push(
              `Upsert images failed '${label}': ${e?.message || String(e)}`
            );
          }
        }
